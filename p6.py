...
def expectimax(gamestate, depth, playerIdx, order):
    ...
    if player == 'P':       # maximizer
        ...
        for move in choices:
            ...
            next_utility, placeholder = expectimax(next_gamestate, depth - 1, (playerIdx + 1) % len(order), order)
        ...
        for move in choices:
            ...
            next_utility, placeholder = expectimax(next_gamestate, depth - 1, (playerIdx + 1) % len(order), order)
            if len(choices) > 0:
                utility = 1 / len(choices) * next_utility  # find average of utilities
        if len(choices) > 0:
            optMove = random.choice(choices)
...
def expecti_max_mulitple_ghosts(problem, k):
    ...
    random.seed(time.time(), version=1)
    ...
    while not gamestate.hasEnded:
        ...
        move = expectimax(gamestate, k, turn % playerNum, order)[1]
